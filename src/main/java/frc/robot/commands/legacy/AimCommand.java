...
public class AimCommand extends CommandBase {
  ...
  public void execute() {
    ...
    if (!fenderShotButton.get()) {
      ...
      //      SmartDashboard.putNumber("Last Vision Target Angle",
      // this.lastVisionTargetAngle.getDegrees());
  ...
  //    if (data.hasTarget) {
  //      final Rotation2d currentTargetAngle = new
  // Rotation2d(turret.getAngle()).rotateBy(Rotation2d.fromDegrees(data.targetAngle));
  //      this.lastVisionTargetAngle =
  // currentTargetAngle.rotateBy(TURRET_TO_ROBOT.getRotation()).rotateBy(swerve.getHeading());
  ...
  //    } else {
  //      final Pose2d goalPose = new Pose2d(new Translation2d(this.lastTargetRange,
  // this.lastVisionTargetAngle), new Rotation2d());
  ...
  //      double a =
  // Math.abs(this.swervePoseAtLastVisionUpdate.getTranslation().getDistance(goalPose.getTranslation()));
  //      double b =
  // Math.abs(this.swervePoseAtLastVisionUpdate.getTranslation().getDistance(currentSwervePose.getTranslation()));
  //      double c =
  // Math.abs(currentSwervePose.getTranslation().getDistance(goalPose.getTranslation()));
  //      Rotation2d deltaAngle = new Rotation2d(Math.acos((Math.pow(a, 2) + Math.pow(b, 2) -
  // Math.pow(c, 2)) / (2 * a * b)));
  //
