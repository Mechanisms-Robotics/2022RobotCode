...
public class TrajectoryController {
    private final static double X_GAIN = 1.25;
    private final static double Y_GAIN = 1.25;
    private final static double THETA_GAIN = 2.0;
    private final static TrapezoidProfile.Constraints HEADING_PROFILE_CONSTRAINTS = new TrapezoidProfile.Constraints(
            2 * Math.PI,
            4 * Math.PI
    );
    ...
    /**
     * Start a trajectory. The calculate functions needs to be called periodically after this function is called.
     *
    ...
    /**
     * Call this function periodically after calling start trajectory. It returns the chassis speeds needed to follow the
     * trajectory.
     *
    ...
    public ChassisSpeeds calculate(Pose2d currentPose) {
        ...
        final var desiredState = (PathPlannerTrajectory.PathPlannerState) trajectory.sample(currentTime);
        return controller.calculate(currentPose, desiredState, desiredState.holonomicRotation);
    }

    /**
     * Return weather the trajectory is finished or not. This is based on weather or not enough time has passed not
     * where the robot is.
     *
