...
public class Turret extends SubsystemBase {
  ...
  static {
    ...
    TURRET_MOTOR_CONFIG.forwardSoftLimitThreshold =
        Units.degreesToFalcon(TURRET_ROTATION_LIMIT_SWITCH, TURRET_GEAR_RATIO);
    TURRET_MOTOR_CONFIG.reverseSoftLimitThreshold =
        Units.degreesToFalcon(TURRET_ROTATION_LIMIT_SWITCH, TURRET_GEAR_RATIO);
    TURRET_MOTOR_CONFIG.initializationStrategy =
  ...
  public void setPosition(double azimuth) {
    turretMotor.set(
        ...
        Units.degreesToFalcon(
            MathUtil.clamp(this.currentAzimuth = azimuth, -BOUND.getDegrees(), BOUND.getDegrees()),
            TURRET_GEAR_RATIO));
  }
  ...
  /**
   * Offsets the position of the turret by specified degrees. Resulting azimuth must be within
   * turret bounds.
   *
   * @param degrees
   */
